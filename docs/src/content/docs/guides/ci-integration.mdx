...
The same two commands work everywhere: install the binary, then
`coverctl check --ci`. PR comments auto-detect from provider env vars.

| System | Install + gate | PR comment env |
| --- | --- | --- |
| GitLab CI | `go install … && coverctl check --ci` | `CI_MERGE_REQUEST_IID` |
...
Minimal Bitbucket PR comment:

```yaml
script:
  - go install go.klarlabs.de/coverctl@latest
  - coverctl check
  - coverctl pr-comment   # uses BITBUCKET_PR_ID
```
