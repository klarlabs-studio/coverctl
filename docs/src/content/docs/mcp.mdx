...
This is **agent-loop coverage governance**, not a Go cover tool. On a Go repo coverctl may invoke `go test`; on Python it invokes `pytest`; then it evaluates `.coverctl.yaml` the same way. Details: [vs native coverage commands](/coverctl/compare/coverctl-vs-native/).
