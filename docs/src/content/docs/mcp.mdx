...
Built for **all sorts of languages** (15 today): Python, TypeScript/JavaScript, Java, Rust, Go, C#, C/C++, PHP, Ruby, Swift, Dart, Scala, Elixir, Shell. This is **agent-loop coverage governance**, not a Go cover tool — coverctl invokes each project's native runner, then evaluates the same `.coverctl.yaml`. Details: [vs native coverage commands](/coverctl/compare/coverctl-vs-native/).
...
| Tool | Mode | Purpose |
| --- | --- | --- |
| `check` | agent + ci | Run tests with coverage and enforce policy. Returns per-domain pass/fail, `failureKind` (`new_regression` / `existing_debt` / `policy_fail` / `pass`), files, warnings. |
...
- **Capability-based execution (agent mode).** Agents express typed capabilities (`packages`, `tags`, `race`, `short`, `run`, `timeout`). Arbitrary `testArgs`, alternate `configPath`, `domains` filters, `coverageScope`, `fromProfile`, and `incremental` are rejected so check cannot hide failing policy, skip tests, or shrink measurement. `coverageScope` remains available in CI/human mode.
...
| `INPUT_REJECTED_PARTIAL_POLICY` | Agent mode received a `domains` filter or `coverageScope`. | Omit `domains` and `coverageScope`; check measures and evaluates every domain in the repository policy. |
| `INPUT_REJECTED_SKIP_VERIFICATION` | Agent mode received `fromProfile: true`. | Omit `fromProfile` so check runs tests. |
| `INPUT_REJECTED_INCREMENTAL` | Agent mode received `incremental: true`. | Omit `incremental`; check tests the full repository. |
