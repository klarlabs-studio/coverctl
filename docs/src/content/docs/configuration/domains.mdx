...
<Aside type="note" title="Not a Go-package wrapper">

`match` entries are path patterns. Go projects often use `./internal/...` because that is how Go tools name packages; Python, TypeScript, Rust, and others use ordinary globs such as `src/api/**`. Same concept, two dialects.
