...
- **Input boundary.** Agent mode accepts typed capabilities (`packages`, `tags`, `race`, `short`, `run`, `timeout`) and rejects arbitrary runner `testArgs` so coverctl cannot be used as a generic command-execution MCP server. Dangerous flags (`--rootdir`, `--cov-config`, `-D`, `-I`, `--require`, `--init-script`, `--node-options`, ...) are also denied as defense in depth. Agent mode cannot select an alternate policy file, filter check to a subset of domains, narrow `coverageScope` (shrinking measurement can hide untested domains), skip the test run via `fromProfile`, or use incremental check (an empty diff auto-passes); repository `.coverctl.yaml` is authoritative and verification must execute tests against the full policy. Rejection responses use a stable schema with `error_code` and agent-actionable `remediation`. CLI invocations from a human terminal are not sanitized; the human is the trust boundary there.
- **Output boundary.** User-controlled identifiers flowing *back* to the agent (filenames in coverage profiles, domain names, package paths) are percent-encoded rather than destructively replaced, so distinct names stay distinct (`src/über.go` does not collapse into another file). Control characters, backticks, and markdown metacharacters cannot render as instructions. Prevents return-trip prompt injection through a hostile PR or attacker-named test file.
...
Canonical intent: [docs/strategy/system-intent.md](docs/strategy/system-intent.md).
